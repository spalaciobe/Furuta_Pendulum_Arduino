...
float Position=10000,Sensor,Motor;
float Balance_Pwm,Position_Pwm;
int Target=10000;   //目标值
unsigned char Send_Count,Flash_Send,Flag_Stop=1;  //停止标志位和上位机相关变量
DATASCOPE data;//实例化一个 上位机 对象，对象名称为 data
float Balance_KP=6,Balance_KD=12,Position_KP=10,Position_KD=10;  //PID系数
float Menu=1,Amplitude1=1,Amplitude2=2,Amplitude3=2,Amplitude4=1; //PID调试相关参数
...
void Adjust(void)
{   
  int Position=2080;  //单击一次用户按键转的角度，1040是360°，所以这里是720°     
  int  temp,temp2;   //临时变量
...
u8 click_N_Double (u8 time)
{
    ...
    if(temp==0)  Forever_count++;   //长按标志位未置1
...
{  
   ...
   static float Position_Least;
    ...
    Position_PWM=Position_Bias*Position_KP/100+Position_Differential*Position_KD; //===位置控制 
...
void setup()   {                
  ...
  attachPinChangeInterrupt(20, READ_ENCODER_B, CHANGE);  //开启外部中断 ////// SE CAMBIO EL 4 POR UN 20 DEBIDO AL CAMBIO DE LIBRERIA VEASE: https://github.com/NicoHood/PinChangeInterrupt
}
/**************************************************************************
函数功能：主循环程序体
入口参数：无
